...
- **Git endpoints** (`server.js`): all under `checkPin` + rate limiter; no-shell `git -C <root>` via `spawnRead` (supports `opts.input` for stdin, e.g. `git apply`), file args validated inside repo root (repo-root itself rejected).
