...
- **External-change watcher** (`public/js/editor-panel.js` → `startOpenFileWatcher()`, booted once in `core.js`): open text buffers (panel + file tabs) snapshot `mtime`/`size` from `GET /api/files/read` (which now returns `mtime`) and are re-statted every 10s (+ on refocus) via `GET /api/files/stat`. Clean buffers auto-reload (`Updated from disk`); dirty ones keep edits and raise a persistent cyan `Changed on disk` indicator (`editor-ext` / `fte-dot.ext` / `has-ext`, click-to-reload via `reloadPanelFile()` / `reloadTabFile()`), cleared by save/reload. Save refreshes the snapshot via a post-write stat so it never self-flags.
...
### Session Persistence
- **tmux sessions**: When tmux is available, WS `session` query param attaches to a `wt-webtun-<port>-{id}` tmux session (`tmuxOwnName()` — note the namespace follows `$PORT`, not the effective listen port, so two flag-port CLI instances still share one namespace). Ownership is tracked, never inferred: every session this process creates is recorded in `ownTmuxSessions`, and shutdown kills exactly those. A box-shared claim file (`os.tmpdir()/webtun-tmux-<port>.json`, pid+token) records the live namespace owner; both sweeps stand down while another live process holds it, and the startup sweep runs only after the port binds. Legacy `wt-webtun-{id}` / `wt-{id}` names are still adopted for reconnect; sweeps reap clientless legacy orphans only when armed, never a foreign port-namespace. Crafted ids can't escape the namespace (`tmuxAdoptableNames()` filters foreign names on attach/resize/delete). Caveats: claim coordination is mutual only once every instance on the box runs this build (pre-fix siblings are invisible to it — their *attached* sessions were always safe from the startup sweep, but their *detached* ones are exposed to it), and changing an instance's port strands its old-namespace sessions (clean once with `tmux kill-session`).
