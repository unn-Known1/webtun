...
function findCloudflared() {
  ...
  // Order matters: canonical install locations FIRST. `findCloudflared()` used
  // to prefer <pkg>/cloudflared and then `./cloudflared`, so running webtun
  // from a directory an attacker controls executed their binary (RCE via CWD
  // hijack) even when a real cloudflared was installed system-wide.
  const candidates = [];
  if (isWin) {
  ...
  // Self-installed copies last — trusted only after the standard paths.
  candidates.push(path.join(PKG_ROOT, name));
  candidates.push(path.join(process.cwd(), name));
  const cwdDir = path.resolve(process.cwd());
  for (const c of candidates) {
    if (!isUsableBinary(c)) continue;
    // Surface the risky case loudly so a hijacked CWD is visible, not silent.
    if (path.dirname(path.resolve(c)) === cwdDir) {
      try { console.warn('[webtun] using cloudflared from the current directory: ' + path.resolve(c)); } catch {}
    }
    return c;
  }
  try {
...
async function downloadCloudflared(onProgress) {
  ...
  try {
    ...
    const okBinary = platform === 'win32' ? (isMz || isElf)
