...
class ZipArchiveReader {
  ...
  async openEntryStream(entry) {
    ...
    try {
      ...
      raw.on('error', () => { try { fh.close(); } catch {} });
      validate.on('close', () => { try { fh.close(); } catch {} });
      if (entry.method === METHOD_STORE) return raw.pipe(validate);
      const inflate = zlib.createInflateRaw();
      inflate.on('error', () => { try { fh.close(); } catch {} });
...
module.exports = { ZipArchiveReader, openZip: (p, o) => ZipArchiveReader.open(p, o) };
