...
// could escape the destination (.., absolute/drive paths, NULs). Callers must
// join the RETURNED name — never the raw fileName — onto the destination.
function safeZipEntryName(fileName) {
  const entryName = String(fileName).replace(/\\/g, '/');
  // Drive prefixes are meaningless to path.normalize on POSIX — reject up front.
  if (/^[A-Za-z]:(\/|$)/.test(entryName) || entryName.includes('\0')) {
    throw zipError('UNSAFE_NAME', 'Invalid zip entry: ' + fileName);
  }
  const entryPath = require('path').normalize(entryName);
  if (entryPath.startsWith('..') || require('path').isAbsolute(entryPath)) {
    throw zipError('UNSAFE_NAME', 'Invalid zip entry: ' + fileName);
  }
  return entryPath;
}
...
// flag set, broken writer) skip validation — there is nothing to check them
// against, and rejecting them would break zips yauzl accepts.
...
class ZipArchiveReader {
  ...
  async openEntryStream(entry) {
    ...
    const fh = await fsp.open(this.zipPath, 'r');
    this._openStreams++;
    let fhClosed = false;
    const closeFh = () => {
      if (fhClosed) return;
