...
// buggy caller must never smuggle `..`, absolute paths, drive prefixes or
// NULs into an archive other tools will extract.
function validateZipName(name) {
  ...
  if (n.split('/').some(seg => seg === '..')) throw new Error('zip entry escapes: ' + n);
  return n;
}
